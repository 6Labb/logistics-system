...
public class DeliveryRouteServiceImpl implements DeliveryRouteService {
    ...
    public DeliveryRouteResponseDto updateDeliveryRoute(UUID deliveryId, UUID id, DeliveryRouteRequestDto requestDto, UserDetailsImpl userDetails) {
        ...
        DeliveryRoute deliveryRoute = findDeliveryWithAuthorization(id, currentRole, currentUserId, "수정");
    ...
    public DeliveryRouteStatusResponseDto updateDeliveryRouteStatus(UUID deliveryId, UUID id, DeliveryRouteStatus status, UserDetailsImpl userDetails) {
        ...
        DeliveryRoute deliveryRoute = findDeliveryWithAuthorization(id, currentRole, currentUserId, "상태 변경");
    ...
    public DeliveryRouteResponseDto deleteDeliveryRoute(UUID deliveryId, UUID id, UserDetailsImpl userDetails) {
        ...
        DeliveryRoute deliveryRoute = findDeliveryWithAuthorization(id, currentRole, currentUserId, "삭제");
...
//        );
    ...
    // 권한 공통 로직
    private DeliveryRoute findDeliveryWithAuthorization(UUID id, Role currentRole, Long currentUserId, String operation) {
        // 관리자와 업체담당자는 전부 가능
        ...
        if (currentRole.equals(Role.HUB_MANAGER)) {
            ...
            return deliveryRouteRepository.findByIdAndToHubId(id, hubManager.getHubId())
                    .orElseThrow(() -> new AccessDeniedException("해당 배송 경로의 " + operation + " 권한이 없습니다."));
        }

        // 배송담당자는 본인의 배송만 가능
        if (currentRole.equals(Role.DELIVERY_AGENT)) {
            return deliveryRouteRepository.findByIdAndDeliveryAgentId(id, currentUserId)
                    .orElseThrow(() -> new AccessDeniedException("해당 배송의 " + operation + " 권한이 없습니다."));
        }

        throw new AccessDeniedException("배송 경로의 " + operation + " 권한이 없습니다.");
    }
}
